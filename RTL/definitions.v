...
/* POST MEM FLIP FLOP DEFINITIONS */

`define MEM_FF_LEN		`REG_WORD_LEN + 1 + `FLOW_MODE_LEN + 1 + `REG_ADDR_LEN + `MEM_ADDR_LEN

`define MEM_FF_OUT		`MEM_FF_LEN
`define MEM_FF_WB			1 + `FLOW_MODE_LEN + 1 + `REG_ADDR_LEN + `MEM_ADDR_LEN
`define MEM_FF_FLOW		`FLOW_MODE_LEN + 1 + `REG_ADDR_LEN + `MEM_ADDR_LEN
`define MEM_FF_BRFL		1 + `REG_ADDR_LEN + `MEM_ADDR_LEN
`define MEM_FF_DEST		`REG_ADDR_LEN + `MEM_ADDR_LEN
`define MEM_FF_JADDR	`MEM_ADDR_LEN
